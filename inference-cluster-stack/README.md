# Air-Gapped Inference Cluster Stack (Identical Node Blueprint)

Docker Compose stack for **identical standalone inference nodes** in separate datacenters.
Each node: 2× NVIDIA H200 141GB (PCIe), Pipeline Parallelism PP=2, full monitoring stack.
...
├── haproxy/
│   └── haproxy.cfg        # Local-only load balancer (optional, enable with --profile optional)
├── registry/
│   ├── config.json        # zot OCI registry config (local image cache)
│   └── data/              # Registry image storage (bind-mounted)
├── prometheus/
...
## Usage (identical on every node)
    ...
    # Optional: enable HAProxy for TLS/rate-limiting
    docker compose --profile optional up -d haproxy
...
4. Pre-pull all images while still online:
        docker compose pull
5. Mirror images to local zot (see Step 6 in the main runbook).

## Notes

- vLLM runs with **Pipeline Parallelism (PP=2)** for PCIe-connected GPUs. See `.env` TP=1 / PP=2.
- The stack is identical on all nodes — no role split, no profiles (except optional HAProxy).
